                                    ...
                                    <CardContent className="p-6 sm:p-8 flex items-start gap-5 pl-8">
                                        ...
                                        <div className="pr-8 flex-1">
                                            {alert.type === 'expired_complete' && (
                                                <><h3 className="font-black text-gray-900 text-lg mb-2">Clearance Alert</h3><p className="text-gray-600 text-sm leading-relaxed"><strong className="text-gray-900 text-base">{alert.item.name}</strong> ({alert.item.brandName}) has<strong className="text-orange-600">{alert.item.nearExpiryQuantity} units</strong>expiring on <span className="text-orange-600 font-bold">{alert.item.expirationDate}</span>. They are now marked as Clearance.</p></>
                                            )}
                                            {alert.type === 'clearance_entered' && (
                                                <><h3 className="font-black text-gray-900 text-lg mb-2">Clearance Alert</h3><p className="text-gray-600 text-sm leading-relaxed"><strong className="text-gray-900 text-base">{alert.item.name}</strong> ({alert.item.brandName}) has<strong className="text-orange-600 px-1 rounded">{alert.item.nearExpiryQuantity} units</strong>expiring on <span className="text-orange-600 font-bold">{alert.item.expirationDate}</span>. They are now marked as Clearance.</p></>
                                            )}
                                            {alert.type === 'order' && (
